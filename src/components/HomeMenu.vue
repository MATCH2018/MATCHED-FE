...
export default {
  ...
  mounted () {
    axios.all([
      ...
      axios.get('http://13.124.173.243:8080/board/notice')])
      .then(axios.spread((roomResponse, noticeResponse) => {
        for (let i = 0; i < 4; i++) {
          this.room['name' + (i + 1)] = roomResponse.data[i].name
          this.Notice['name' + (i + 1)] = noticeResponse.data[noticeResponse.data.length - i - 1].title
        }
      }))
  },
  methods: {
