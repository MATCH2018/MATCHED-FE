<template>
    <div class="password-reset-container">
        <div class="password-reset-form">
            ...
            <div v-if="!isCodeSent">
                <button class="btn btn-primary" @click="sendCode">인증링크 전송</button>
            </div>
            <div v-else>
                <input type="email" id="code" v-model="code" placeholder="인증번호"/>
                <button class="btn-primary1" @click="VerifyCode">확인</button>
...
export default {
  ...
  methods: {
    ...
    VerifyCode () {
      axios.post('url', { code: this.code })
        .then(response => {
          alert('이메일 인증이 성공했습니다.')
          localStorage.setItem(this.IsVerify, true)
